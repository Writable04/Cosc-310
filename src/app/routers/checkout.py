...
def checkout(
    ...
    if body.redeem_points:
        ...
        if current_points > 0:
            ...
            if full_discount >= cart_amount:
                # Points cover the full subtotal — calculate rollover
                points_discount = cart_amount
                points_used = math.ceil(cart_amount * POINTS_TO_DOLLAR)  # exact points needed
    ...
    discounted_subtotal = round(cart_amount - points_discount, 2)
    ...
    total_before_tax = round(discounted_subtotal + delivery_fee, 2)

    # Send original subtotal + delivery to payment (tax calculated on full subtotal)
    total_with_delivery = round(cart_amount + delivery_fee, 2)
    ...
    # Earn 1 point per $1 of cart subtotal (before discount and fees)
